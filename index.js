...
const screen = blessed.screen({
  smartCSR: true,
  title: "SOUIY  Fishing Frenzy Auto ",
  fullUnicode: true,
  mouse: true,
});
...
figlet.text("SOUIY", { font: "ANSI Shadow" }, (err, data) => {
    ...
    if (!err) {
      ...
      asciiBanner = "{center}{bold}SOUIY{/bold}{/center}";
    }   
  const descriptionText = "{center}{bold}{bright-yellow-fg}✦ . ── Fishing Frenzy Auto Bot!! ── .✦\n✦. FOLLOW TIKTOK : AirdropRefferal (Souiy1) .✦{/bright-yellow-fg}{/bold}{/center}";
...
addLog(`{bright-yellow-fg}Satukan Solidaritas Bantu Palestina!
: ${is5xEnabled ? 'OK' : 'OK'}{/bright-yellow-fg}`);
addLog(`{bright-yellow-fg}https://digital.dompetdhuafa.org/donasi/jagapalestina : ${is5xEnabled ? 'OK' : 'OK'}{/bright-yellow-fg}`);
